/**
 * Durandal 2.1.0 router fork.
 * Allows to activate routes marked with `dialog: true` in dialogs.
 * User `owner` configuration to set page that should be loaded behind the dialog.
 *
 * For example, given this config:
 *
 * router.map([
 *   {route: 'home', moduleId: 'viewmodels/home'},
 *   {route: 'about', moduleId: 'viewmodels/about'}
 *   {route: 'singin', moduleId: 'viewmodels/signin', dialog: true, owner: 'home'}
 * ])
 *
 * If you navigate to '#home' and then navigate to '#singin', you should see a dialog with a "viewmodels/signin" module in it and "viewmodels/home" module underneath
 *
 * If you navigate to '#about' and then navigate to '#singin', you should see a dialog with a "viewmodels/signin" module in it and "viewmodels/about" module underneath
 *
 * If you navigate to '/#signin', you should see a dialog with a "viewmodels/signin" module in it and "viewmodels/home" module underneath (as set in owner)
 */

define(['durandal/system', 'durandal/app', 'durandal/activator', 'durandal/events', 'durandal/composition', 'plugins/history', 'plugins/dialog', 'knockout', 'jquery'], function (system, app, activator, events, composition, history, dialog, ko, $) {
	var optionalParam = /\((.*?)\)/g;
	var namedParam = /(\(\?)?:\w+/g;
	var splatParam = /\*\w+/g;
	...
	var createRouter = function () {
		var queue = [],
            ...
            activeDialog = ko.observable(), //viewmodel of active dialog
            activeDialogInstruction = ko.observable();
		...
		var router = {
			...
			activeDialogInstruction: activeDialogInstruction
		};
		...
		function configureRoute(config) {
			...
			router.route(config.routePattern, function (fragment, queryString) {
				var paramInfo = createParams(config.routePattern, fragment, queryString),
                    ...
                    isDialogRoute = config.dialog === true,
                    instruction = {
                    	fragment: fragment,
				...
				if (activeDialogInstruction() && activeDialogInstruction().config.moduleId == instruction.config.moduleId) {
					system.log('Reusing current dialog (same', instruction);
					activeDialogInstruction(instruction);
					if (activeDialog().activate) {
						activeDialog().activate.apply(activeDialog(), instruction.params);
					}
				...
				if (activeDialog()) {
					...
					dialog.close(activeDialog());
					activeDialog(null);
					activeDialogInstruction(null);
				}

				if (!isDialogRoute) {
					queueInstruction(instruction);
				} else {
					var ownerHash = config.owner || '#',
                        ownerConfig = ko.utils.arrayFirst(router.routes, function (r) {
                        	return r.hash == ownerHash || r.route == ownerHash;
                        });

					system.acquire([ownerConfig.moduleId, config.moduleId]).then(function (modules) {
						var ownerInstance = modules[0],
                            instance = modules[1], //dialog
                            ownerInstruction = {
                            	fragment: ownerHash,
                            	queryString: "",
                            	config: ownerConfig,
                            ...
                            showDialog = function () {
                            	//owner has navigated
                            	completeNavigation(instance, instruction, 'rootRouter');
                            	history.navigate(instruction.fragment, false);
                            	activeDialog(instance);
                            	activeDialogInstruction(instruction);
                            	var activeHash = router.activeInstruction().fragment;

                            	return dialog
                                    .show(instance, params)
                                    .always(function () { //when dialog is closed
                                    	completeNavigation(ownerInstance, ownerInstruction, 'rootRouter');
                                    	history.navigate(activeHash, false);
                                    	activeDialog(null);
                                    	activeDialogInstruction(null);
                                    });
                            };
						...
						if (!router.activeInstruction()) {
							queueInstruction(ownerInstruction);
							var subscription = router.on('router:navigation:complete', function () {
								subscription.off();
								showDialog();
							});
